  ...
  try {
    ...
    if (exist) {
      ...
      return res
        .status(200)
        .json(
          new ApiResponse(200, createSubscription, "Subscribed successfully")
        );
    }
  } catch (error) {
...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (subscribers.length == 0) {
      return res
        .status(200)
        .json(new ApiResponse(200, subscribers, "No subscribers"));
    }
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, subscribed, "Subscriptions fetched successfully")
      );
  } catch (error) {
    throw new ApiError(500, error.message, error);
  }
