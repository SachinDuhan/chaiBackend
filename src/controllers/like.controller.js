...
const toggleVideoLike = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (exist) {
            const deleteLike = await Like.deleteOne({video: videoId, likedBy: user?._id});

            if (deleteLike.deletedCount === 0) {
                throw new ApiError(404, "Like not found or not deleted");
            }

            return res.status(200).json(new ApiResponse(200, deleteLike, "Like deleted successfully"))
        } else if (!exist)  {
            ...
            return res.status(200).json(new ApiResponse(200, createLike, "Like created successfully"))
        }
    } catch (error) {
...
const toggleCommentLike = asyncHandler(async (req, res) => {
    ...
    try {
        const exist = await Like.findOne({comment: commentId, likedBy: user?._id});
        

        if (exist) {
            const deleteLike = await Like.deleteOne({comment: commentId, likedBy: user?._id});
            ...
            const createLike = await Like.create({comment: commentId, likedBy: user?._id});
...
const toggleTweetLike = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (exist) {
            const deleteLike = await Like.deleteOne({tweet: tweetId, likedBy: user?._id});
...
const getLikedVideos = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (likedVideos.length == 0) {
            return res.status(200).json(new ApiResponse(200, likedVideos, "No like videos"))
        }
