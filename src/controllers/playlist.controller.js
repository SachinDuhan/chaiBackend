...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  ...
  try {
    const addVid = await Playlist.updateOne(
      { _id: playlistId },
      { $push: { videos, videoId } }
    );
    ...
    if (!addVid || addVid.modifiedCount == 0) {
      throw new ApiError(
        500,
        "There was a problem adding the video to the playlist"
      );
    }

    return res
      .status(200)
      .json(new ApiResponse(200, addVid, "Video added to playlist"));
  } catch (error) {
    throw new ApiError(500, error.message, error);
  }
});
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!deleteVideo || deleteVideo.modifiedCount == 0) {
      throw new ApiError(
        500,
        "There was a problem removing the video from the playlist"
      );
    }
