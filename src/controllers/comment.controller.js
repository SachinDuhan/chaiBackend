...
const getVideoComments = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const commentsPaginate = await Comment.aggregatePaginate(comments, {
      page: parseInt(page),
    ...
    if (commentsPaginate.docs.length == 0) {
      return res
        .status(200)
        .json(new ApiResponse(200, commentsPaginate, "No Comments"));
    }
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, commentsPaginate, "Comments loaded successfully")
      );
  } catch (error) {
